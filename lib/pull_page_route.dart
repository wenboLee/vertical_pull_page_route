...
const double _kBackGestureWidth = 120.0;
...
class PullPageRoute<T> extends PageRoute<T> {
...
//      return VoiceCupertinoFullscreenDialogTransition(
...
//      );
//    } else {
    return VoiceCupertinoPageTransition(
      primaryRouteAnimation: animation,
      secondaryRouteAnimation: secondaryAnimation,
      linearTransition: linearTransition,
...
class VoiceCupertinoPageTransition extends StatelessWidget {
...
//    debugPrint("  handleDragUpdate -----------> $details");
    _backGestureController
        .dragUpdate(_convertToLogical(details.primaryDelta / context.size.height));
  }
  ...
  void _handleDragEnd(DragEndDetails details) {
    ...
    /// 这里计算了下滑的加速度
    _backGestureController
        .dragEnd(_convertToLogical(details.velocity.pixelsPerSecond.dy / context.size.height));
  ...
  Widget build(BuildContext context) {
    ...
    double start = MediaQuery.of(context).size.width - dragAreaWidth;
    return Listener(
